...
for epoch in range(num_epochs):
    ...
    for idx in indices:
        ...
        input_seq = torch.tensor(X_train[idx], dtype=torch.float32).view(sequence_length, input_size).unsqueeze(0)
        target_note = torch.tensor(y_train[idx], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
    ...
    for i in range(len(X_test)):
        ...
        target_note_val = torch.tensor(y_test[i], dtype=torch.float32).unsqueeze(0).unsqueeze(0)
...
axs[0].plot(range(N+1, num_epochs+1), train_loss_history[N:], label='Train Loss')
axs[0].plot(range(N+1, num_epochs+1), val_loss_history[N:], label='Val Loss') 
...
axs[1].set_yscale('log')  # Cambia la scala dell'asse y in una scala logaritmica
